...
class ManufacturerSearchTests(TestCase):
    def setUp(self):
        ...
        self.manufacturer3 = Manufacturer.objects.create(name="Tesla", country="USA")
    ...
    def test_manufacturer_search_by_partial_name(self):
        response = self.client.get(reverse("taxi:manufacturer-list") + '?name=Tes')
        self.assertEqual(response.status_code, 200)
...
class DriverSearchTest(TestCase):
    ...
    def test_driver_search_by_partial_username(self):
        response = self.client.get(reverse("taxi:driver-list") + '?username=Driver2')
        self.assertEqual(response.status_code, 200)
    ...
    def test_driver_search_no_results(self):
        response = self.client.get(reverse("taxi:driver-list") + '?username=Driver4')
        self.assertEqual(response.status_code, 200)
        self.assertNotContains(response, "Driver1")
        self.assertNotContains(response, "Driver2")
        self.assertNotContains(response, "Driver3")
...
class CarSearchTest(TestCase):
    def setUp(self):
        ...
        self.car2 = Car.objects.create(model="F150", manufacturer=self.manufacturer2)
        self.car3 = Car.objects.create(model="Model S", manufacturer=self.manufacturer3)
